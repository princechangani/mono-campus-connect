...
export default function AdminFacultyPage() {
  ...
  const load = () => {
    api.get("/admin/users/role/FACULTY").then(r => setFaculty(r.data)).catch(() => {}).finally(() => setLoading(false));
  };
  useEffect(() => { load(); }, []);
  ...
  const filtered = faculty.filter(f =>
    `${f.firstName} ${f.lastName} ${f.email} ${f.facultyId ?? ""}`.toLowerCase().includes(search.toLowerCase())
  );

  return (
    <DashboardLayout title="Faculty">
      <div className="flex items-center justify-between mb-6 flex-wrap gap-3">
        <h2 className="text-xl font-semibold text-gray-800">Faculty ({faculty.length})</h2>
      ...
      <div className="card overflow-x-auto">
        {loading ? (
          ...
          <p className="text-gray-400 text-sm text-center py-10">No faculty found</p>
        ) : (
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-gray-100">
                ...
                <th className="text-left py-2 px-3 text-gray-500 font-medium">Faculty ID</th>
            ...
            <tbody>
              {filtered.map(u => (
                <tr key={u.id} className="border-b border-gray-50 hover:bg-gray-50">
                  ...
                  <td className="py-2.5 px-3 text-gray-500">{u.facultyId ?? "—"}</td>
        ...
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-xl shadow-xl w-full max-w-md">
            <div className="flex items-center justify-between p-5 border-b">
              <h3 className="font-semibold text-gray-800">{editing ? "Edit Faculty" : "Add Faculty"}</h3>
            ...
            <form onSubmit={handleSubmit(onSubmit)} className="p-5 space-y-3">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div><label className="block text-xs font-medium text-gray-700 mb-1">Faculty ID</label><input {...register("facultyId")} className="input" /></div>
